...
export default function WikiCard({ page }: Props) {
  ...
  return (
    <Link
      href={`/wiki/${page.slug}`}
      className="group flex flex-col p-5 bg-slate-800/20 border border-slate-700/20 rounded-2xl
               hover:border-indigo-500/20 hover:bg-slate-800/40 hover:shadow-lg hover:shadow-indigo-500/5
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="min-w-0">
          <h3 className="text-[14px] font-semibold text-slate-200 group-hover:text-white leading-snug transition-colors line-clamp-2">
            {page.title}
          </h3>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-800/50">
        ...
        {page.tags && page.tags.length > 0 && (
          <div className="flex gap-1">
            ...
            {page.tags.length > 2 && (
              <span className="px-2 py-0.5 rounded-md bg-slate-700/20 text-[10px] text-slate-600">
                +{page.tags.length - 2}
              </span>
            )}
          </div>
